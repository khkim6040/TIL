...
RDS의 parameter group에 설정된 `max_connections` 값은 `LEAST({DBInstanceClassMemory/25165760},12000)` 공식으로 계산된다. 현재 사용 중인 `db.t3.micro` 인스턴스의 메모리는 1GiB(=1,074,000,000B)이므로, 이를 25165760B로 나누면 약 **42**가 나온다.
...
백엔드 서버가 시작될 때, TypeORM은 이는 매번 쿼리마다 커넥션을 새로 만드는 오버헤드를 줄이기 위해, DB 인스턴스에 미리 여러 개의 커넥션을 맺어두는 커넥션 풀을 생성한다.

TypeORM의 기본 커넥션 풀 크기는 **최대 10개**로 설정되어 있다. 상황에 따라 동적으로 조절되지만, 최대 10개까지 커넥션을 생성할 수 있다.
...
아래와 같이 TypeORM 설정에서 `extra` 옵션의 `connectionLimit`을 사용하면 커넥션 풀의 최대 크기를 제한할 수 있다.
