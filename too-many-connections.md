...
흠.. 42개면 29개보다 많을 텐데 뭔가 내가 모르게 DB 인스턴스에서 예약해 사용하는 커넥션들이 있을 수도 있겠다. _<- 아니었다_
...
이상한 점은 함께 connection size를 1로 줄였던 다른 개발서버는 정상적으로 응답을 해줬다는 점이다. 왜 이럴까..
...
다른 해결책으로는, 트랜잭션 안으로 READ를 집어넣어 동일한 커넥션이 READ를 수행하게 만들든가, 트랜잭션 문 바깥으로 READ를 옮겨서 `queryRunner.release()`가 실행된 후에 READ를 해 주는 방법이 있다. 후자가 더 나아 보이는데, 전자는 트랜잭션 내부 READ 시 에러가 발생한다면 트랜잭션 내부에서 정상적으로 업데이트 되었던 변경들도 모두 롤백이 될 수 있기 때문이다. 트랜잭션 문에서 리턴 함수를 뒤로 빼주는 식으로 변경할 것 같다.