...
여러 프로그램을 큐 형태로 실행하는 배치 방식으로 진화. 프로세서가 일하면서 입출력을 백그라운드에서 진행하는 direct memory access(DMA) 방법.
...
키보드와 같은 외부 하드웨어와 인터럽트를 기반으로 입력을 받을 수 있다. 컴퓨터가 아직까지 귀했던지라 여러 유저의 접속과 사용을 조율한다. 아직까지는 개인(Personal) 컴퓨터가 아니다.
