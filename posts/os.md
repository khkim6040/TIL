...
#### 1.2.3 Portability
ARM, AMD 등 서로 다른 CPU 별로 어플리케이션 코드를 작성해야 한다면 참 힘들 것이다. 같은 코드를 대부분의 하드웨어에서 동일하게 실행할 수 있을 떄 이식성(Portability)이 좋다고 한다. 이를 위해서는 계층 간 추상화된 규약을 잘 따라야 한다. 

하드웨어 - 운영체제 - 어플리케이션으로 계층이 구성되어 있을 때, '운영체제는 ~~역할을 하는 기능을 노출해야 한다'라는 약속을 따른다면, 어플리케이션 개발자는 본인의 어플리케이션이 어떤 운영체제를 사용하는지 상관하지 않고 해당 기능을 사용함으로써 어플리케이션을 개발할 수 있다.

운영체제 내부에는 하드웨어 차이를 숨기기 위한 하드웨어 추상화 계층(HAL)이 존재한다. 이 계층이 하드웨어 차이를 흡수해주어 운영체제의 상위 계층은 특정 CPU나 디바이스에 의존하지 않고 구현될 수 있다. 이 나머지 부분은 어플리케이션에 시스템 콜 인터페이스의 형태로 API를 노출한다.
