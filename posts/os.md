...
그렇게 하기 위해 소프트웨어가 아닌 하드웨어의 도움을 받는다. 일정 주기가 지날 때마다 커널 모드로 세팅한 후 커널을 부르는 하드웨어 타이머가 그 역할을 한다. 어떤 프로그램이 돌아가더라도, 하드웨어 타이머가 작동해서 커널이 불렸을 때 커널이 어떤 작업을 할 지 마음대로 정할 수 있게된다.
