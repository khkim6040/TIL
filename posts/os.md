...
파일 디스크립터는 정수 인덱스이고, 프로세스별 파일 디스크립터 테이블이 커널에 존재한다. 이 테이블이 실제 디바이스 드라이버를 가리킨다.
...
- `close()`: 사용 후에는 garbage collecting과 사용자 추적을 위해 파일 디스크립터를 닫아야 한다.
...
[1.2.3](#123-Portability)에서 언급한 HAL(Hardware Abstraction Layer)와 파일 디스크립터는 동일한 역할을 한다. 아랫단의 다양성을 잘 포장해 윗 단에 공통된 인터페이스를 노출시켜준다. 이 경우, 윗 단인 유저 프로세스는 파일 디스크립터 뒤에 있는 상대가 파일인지 소켓인지 파이프인지 몰라도 read/write만 호출하면 된다. 디바이스가 추가되어도 유저 프로그램을 수정할 필요가 없으므로 유저 프로그램의 portability가 올라간다.
