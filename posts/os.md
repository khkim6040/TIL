...
__아래는 내 생각__
당연하다 생각했던 부분이 사실은 정교하게 설계된 부분이었다는게 놀라웠다. 특히 일반적인 커널 -> 유저 모드로 전환될 때 PC, esp를 빼내는 과정을 재활용해서 프로세스가 시작될 때 아다리가 맞게 한 부분이 재밌었다. 그런데 재활용을 위해서 이런 과정을 거쳐야 했을까? 프로세스 시작 이벤트는 인지할 수 있고, 할당한 유저 스택 주소들도 커널이 알고 있을테니 그냥 그 값들로 PC, esp 세팅하면 곧바로 프로세스를 시작할 수 있었을 텐데 말이다. 근데 이렇게 되면 main()의 끝에 다른 함수들처럼 return을 달지 못하고 exit()을 달아야 한다. 그래서 프로세스가 끝났을 때 안전하게 종료시키는 부분이 애매하게 될 것같긴 하다.
...
그 대신 하드웨어 초기화 및 디스크에 저장된 부트로더를 실행까지만 시켜주어 용량이 작은 BIOS를 ROM에 둔다. 이후 부트로더가 실행되고 비슷하게 운영체제를 메모리로 옮겨와 실행하게 된다.
...
부트로더는 디스크에 저장되기 때문에 업데이트가 가능하고, 파일 시스템을 이해하며, 어떤 커널을 어디에 적재할지 결정한다. 즉 BIOS는 최대한 작고 멍청하게 만들어서 부트로더 발사대로 삼고, 부트로더가 운영체제를 실행하기 전 최종 점검을 한다고 생각하면 될 것같다.  

### 2.10 Case Study: Virtual Machines
커널이 하드웨어 자원을 가상화 해 유저 레벨로 제공해주는 것을 **가상 머신**이라 한다. 가상 머신에서 돌아가는 게스트 운영체제는 자신이 실제 하드웨어 위에서 동작한다고 생각하며 호스트 운영체제라고 생각한다. 자신이 모든 자원을 다 다룰 수 있다고 생각하지만, 이는 모두 호스트 운영체제의 관리 하에서 시뮬레이션 되는 결과다. 
