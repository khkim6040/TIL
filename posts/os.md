...
일부 레지스터는 하드웨어가 인터럽트/예외 진입 시 자동으로 커널 스택에 push하고, 나머지 레지스터들은 커널이 인터럽트 진입 코드에서 명시적으로 저장한다. 커널 핸들러가 완료돈 후 저장된 상태를 복구한 후 유저 프로세스로 복귀한다. 현재 커널 스택의 스택 포인터는 하드웨어 레지스터에 저장되어 있으며, 인터럽트 진입 시 이 포인터를 기준으로 레지스터 상태가 저장된다.
