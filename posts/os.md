...
- 여러 프로그램을 동시에 실행시킬 때 유저가 모든 프로그램이 동시에 실행되고 있다고 느낄 수 없게되기 때문이다.
...
- 인터럽트: 외부 이벤트에 의한 시그널이다. CPU가 실행되고 있는 명령을 중지하고 다른 장소에 현재 실행 상태를 저장한 후, 발생한 인터럽트에 맞는 미리 등록된 핸들러를 호출한다. 프로세서간 소통하는 IPC(Inter Process Communication)도 인터럽트 형식이다.
- 예외: 유저 프로그램에 의해 발생한 하드웨어 이벤트다. 유저 프로그램이 권한 없는 기능을 실행했거나, 0으로 나누기, 할당된 메모리 영역 바깥을 참조 등 하면 안될 행동을 했을 때 발생한다. 하드웨어가 예외 상황을 인지하고 상태 저장 후 커널에게 알려서 상황에 맞는 핸들러를 실행하도록 한다. 보통 해당 프로그램을 종료시킴으로써 처리된다. 
위와 같은 경우 말고, 예외를 통해 어떤 기능을 수행할 수도 있다. 가상화가 대표적 예시다. 작게는 예외를 통해 소수점 연산을 하는 것이다. 소수점 연산을 하지 못하는 프로세서는 소수점 연산 명령이 들어왔을 때 예외를 던질 것이다. 이 때 커널은 소프트웨어 레벨에서 소수점 연산을 수행하고 반환한다. 유저 프로그램은 프로세서가 정상적으로 소수점 연산을 했다고 착각한다. 또한, 넓게는 가상 머신에 적용될 수 있다. 호스트 머신에서 유저 모드로 돌아가고 있는 게스트 운영체제가 특권 명령(Privileged Instruction)을 실행했을 때, 호스트 머신에 예외가 발생한다. 정교하게 하드웨어를 애뮬레이션 한 호스트 머신은 게스트 운영체제가 실행한 명령을 수행하고 이를 반환한다. 게스트 운영체제는 자신이 하드웨어를 사용하는 정상적인 호스트 운영체제라고 생각한다.
