...
- **프로그램은 기계어가 아니라 커맨드를 담은 파일일 수 있다:** `#! python3`로 시작하는 쉘 스크립트를 본 적이 있을 것이다. 이건 이 스크립트 파일을 프로그램으로써 실행하겠다고 쉘에게 알려주는 것이다. 쉘은 이 특수한 알림(`#!`)을 이해하고 그에 맞는 인터프리터(여기서는 `python3`)를 실행시켜 파일을 통째로 인터프리터에게 전달해준다.
