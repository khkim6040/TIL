...
- 원자적 프로세서 상태 전환: 모드를 바꾸면 실행되는 코드 환경도 달라진다. 유저 모드에서 실행되고 있던 유저 프로그램이 커널 모드로 바뀌면 상관없는 커널 코드를 실행하게 된다. 이 때, 메모리와 레지스터 값들이 변경되는데, 이 과정이 진행되는동안 다른 과정이 끼어들어 메모리 값이 바뀌거나 다른 코드가 실행된다면 문제가 발생할 수 있다. 따라서 상태 전환 중에는 다른 과정이 끼어들 수 없어야 한다.
...
인터럽트 벡터 핸들러는 포인터의 배열이다. 각 포인터는 특정 인터럽트 핸들러 함수를 가리킨다. 하드웨어가 인터럽트 혹은 예외를 발생시킬 때 어떤 핸들러를 실행해야 하는지 특정 레지스터에 핸들러 값을 설정한다. 커널은 그 레지스터 값을 읽어 지정된 핸들러를 실행시킨다. 시스템 콜은 유저 프로그램이 자발적으로 호출한 것으로, 유저 프로그램이 어떤 핸들러 실행을 원하는지 명시한다. 
