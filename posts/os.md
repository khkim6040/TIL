...
`fork()`는 이 과정에서 실제로는 주소 공간의 물리 페이지를 즉시 복사하지 않고, 페이지 테이블을 **공유**한 상태로 시작한다. 이후 어느 한쪽이 쓰기를 시도할 때 해당 페이지만 복사하는 COW(Copy-On-Write) 기법을 통해 시간과 공간을 절약한다. 이와 같은 효율적인 메모리 관리 방법에 대해서는 나중에 더 자세히 알아볼 것이다. 
