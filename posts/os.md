...
- 예외: 예외: 프로그램 실행 중 비정상적인 조건이 발생했을 때 CPU가 생성하는 동기적 이벤트다. 유저 프로그램이 권한 없는 기능을 실행했거나, 0으로 나누기, 할당된 메모리 영역 바깥을 참조 등 하면 안될 행동을 했을 때 발생한다. 하드웨어가 예외 상황을 인지하고 상태 저장 후 커널에게 알려서 상황에 맞는 핸들러를 실행하도록 한다. 보통 해당 프로그램을 종료시킴으로써 처리된다.
...
__내 생각: 당연하다 생각했던 부분이 사실은 정교하게 설계된 부분이었다는게 놀라웠다. 특히 일반적인 커널 -> 유저 모드로 전환될 때 PC, esp를 빼내는 과정을 재활용해서 프로세스가 시작될 때 아다리가 맞게 한 부분이 재밌었다. 그런데 재활용을 위해서 이런 과정을 거쳐야 했을까? 프로세스 시작 이벤트는 인지할 수 있고, 할당한 유저 스택 주소들도 커널이 알고 있을테니 그냥 그 값들로 PC, esp 세팅하면 곧바로 프로세스를 시작할 수 있었을 텐데 말이다. 근데 이렇게 되면 main()의 끝에 다른 함수들처럼 return을 달지 못하고 exit()을 달아야 한다. 그래서 프로세스가 끝났을 때 안전하게 종료시키는 부분이 애매하게 될 것같긴 하다.__
