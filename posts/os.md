...
- Throughput: 응답 시간과 비슷한 개념으로, 시간 당 얼마나 많은 작업을 처리할 수 있느냐이다. 응답 시간과 처리량은 일반적으로 반비례 관계이지만, 항상 그렇지는 않다. 
- Performance Predictability: 성능이 사용자가 기대하는대로 유지되는지 여부이다. 응답에 즉시 반응하다가 1% 확률로 10초가 걸리는 운영체제보다 매 응답에 0.1초가 걸리는 운영체제가 예측하기 쉬우니 사용하기 더 낫다는 의미다.

#### 1.2.5 Adoption
운영체제가 얼마나 어플리케이션과 하드웨어와 호환 가능한지도 운영체제의 성공에 중요하다. Portability와 비슷한 개념이다. 여기서는 그것 외에도 운영체제의 소유권을 어떻게 할 것인지도 의논했다. 애플, 윈도우즈와 같이 운영체제를 공개하지 않는 방식과, 리눅스처럼 완전히 공개하는 방식 두 가지가 있다. 전자는 믿음직스럽지만 호환성, 확장성이 떨어질 수 있다. 후자는 그 반대로 호환성이 좋지만, 그래서 운영체제가 하나로 유지되지 않고 여러 버전으로 분기되어 유저 풀이 흩어질 수 있다. 
