...
4. 인터럽트 핸들러가 실행을 마친 후 커널은 저장했던 레지스터를 복구한 후, 하드웨어가 저장했던 레지스터까지 복구하고 모드를 전환한다.
...
- PCB를 생성하고, 초기 레지스터 상태(PC, SP 등)를 설정한 뒤 스케줄러를 통해 해당 PCB를 실행 대상으로 선택한다.
    - 다만, 첫 번째 실행 떄는 참고할 예전에 실행했던 코드가 없어서 참고할 주소가 없다. 이를 프로세스 본문을 **호출**하는 가짜 함수를 호출함으로써 대체한다. 자세한 건 아래 본문 참고
...
A. 가상 머신을 통해서라면 운영체제가 프로세스 안에서 실행되는 것이 가능하다. 운영체제가 하드웨어 자원을 가상화해서 게스트 운영체제를 자신 위에, 혹은 프로세스(컨테이너) 위에서 실행할 수 있다. 이 게스트 운영체제는 자신이 하드웨어를 통제하는 유일한 운영체제라고 생각한다. 실은 호스트 운영체제가 하드웨어 가상화 기능을 이용해 게스트 운영체제의 특권 동작을 가로채거나 직접 실행하도록 제어한다.
