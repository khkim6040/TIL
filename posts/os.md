...
시스템 콜은 제한된 권한 아래서 동작하는 유저 프로세스가 권한 밖의 행위를 할 때 호출하는 함수로, 프로세스 생성, 파일 쓰기, 파일 읽기 등이 있다. 시스템 콜은 커널 단에서 구현되어 API를 유저 어플리케이션에 노출된다. 유저 프로세스는 적절한 입력값을 넣고 시스템 콜을 호출한 뒤 결과를 받아서 사용한다. 여느 라이브러리를 사용하는 것과 다르지 않다.
...
- (3, 커널 쪽 도우미) 중요한 부분이다. 일단, 덮어쓸수도 있는 다른 레지스터 값들을 저장한다. 주소, 값, 길이 등 시스템 콜 번호를 포함한 모든 파라미터 유효성 검증을 한다. 그리고 유저 프로그램이 사용하는 가상 주소 값을 커널이 사용하는 물리 주소 값으로 변환한다. 다 유효하다면 커널의 시스템 콜 핸들러를 호출한다.
